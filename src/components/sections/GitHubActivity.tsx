...
// Month labels with approximate week offsets (52w = ~1yr, starting Jun 2025)
const MONTHS = [
  { label: 'Jun', week: 0  },
  { label: 'Jul', week: 5  },
  { label: 'Aug', week: 9  },
...
// Deterministic activity level — no Math.random() so SSR and client match
...
// Pre-compute once at module level — stable across renders
...
export default function GitHubActivity() {
  ...
  useEffect(() => {
    ...
    const cells = sectionRef.current.querySelectorAll('.heat-cell')
    ...
    const ctx = gsap.context(() => {
      ...
      // Stat counters
      const counters = [
        { ref: stat1Ref, target: person.stats.contributions },
        { ref: stat2Ref, target: person.stats.repos         },
        { ref: stat3Ref, target: person.stats.yearsCoding   },
      ]
      counters.forEach(({ ref, target }) => {
        ...
        gsap.to(obj, {
          ...
          onUpdate() { if (ref.current) ref.current.textContent = Math.floor(obj.val) + '+' },
          onComplete() { if (ref.current) ref.current.textContent = target + '+' },
          scrollTrigger: { trigger: sectionRef.current, start: 'top 68%', once: true },
        })
      })

      // Heatmap cells fill left-to-right
      gsap.from(cells, {
        opacity: 0,
        duration: 0.1,
        ease: 'power1.out',
        stagger: { from: 'start', amount: 1.2 },
        scrollTrigger: { trigger: '.heatmap-wrap', start: 'top 82%', once: true },
      })
    }, sectionRef)
        ...
        <div className="github-layout">
          ...
          <div>
            <div className="heatmap-wrap">
              ...
              >
                <span className="t-label" style={{ color: 'var(--text-secondary)', marginRight: 4 }}>
                  Less
                </span>
                ...
                <span className="t-label" style={{ color: 'var(--text-secondary)', marginLeft: 4 }}>
                  More
                </span>
              </div>
            </div>
